...
CAr::CAr(void* lpBuf, unsigned int nBufSize)
{
	if (lpBuf)
	{
		m_nMode = load;
...
void CAr::Read(void* lpBuf, unsigned int nSize)
{
	if (nSize == 0)
		return;
...
void CAr::Write(const void* lpBuf, unsigned int nSize)
{
	if (nSize == 0)
		return;
...
void CAr::Reserve(unsigned int nSize)
{
	if (nSize <= nGrowSize)
		return;
...
void CAr::CheckBuf(unsigned int nSize)
{
	if (m_lpBufCur + nSize > m_lpBufMax)
	{
		unsigned int uOffset = m_lpBufCur - m_lpBufStart;

		//		unsigned int nExtension	= nGrowSize * ( nSize / nGrowSize + 1 );
		unsigned int nExtension = m_nBufSize * 2;
...
void CAr::ReelIn(unsigned int uOffset)
{
#if 1
	if (m_nBufSize > nGrowSize)
	{
